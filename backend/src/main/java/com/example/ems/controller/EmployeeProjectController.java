...
public class EmployeeProjectController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<EmployeeProject> getEmployeeProjectById(
        @PathVariable Long id
    ) {
        return employeeProjectService
            .getEmployeeProjectById(id)
            .map(ResponseEntity::ok)
            .orElse(ResponseEntity.notFound().build());
    }

    @PostMapping
    public EmployeeProject createEmployeeProject(
    ...
    @PutMapping("/{id}")
    public ResponseEntity<EmployeeProject> updateEmployeeProject(
        @PathVariable Long id,
        @RequestBody EmployeeProject employeeProjectDetails
    ) {
        try {
            EmployeeProject updated =
